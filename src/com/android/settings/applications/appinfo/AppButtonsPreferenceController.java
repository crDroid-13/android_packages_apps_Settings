...
public class AppButtonsPreferenceController extends BasePreferenceController implements
                ...
                if (mAppEntry.info.enabled && !isDisabledUntilUsed()) {
                    ...
                    requireAuthAndExecute(() -> {
                        mMetricsFeatureProvider.action(
                                mActivity,Add commentMore actions
    ...
    private void requireAuthAndExecute(Runnable action) {
        if (Utils.isProtectedPackage(mContext, mAppEntry.info.packageName)) {
            AppInfoDashboardFragment.showLockScreen(mContext, () -> action.run());Add commentMore actions
        } else {
            action.run();
        }
    }
    ...
    public void handleDialogClick(int id) {
        switch (id) {
            case ButtonActionDialogFragment.DialogType.DISABLE:
                 requireAuthAndExecute(() -> {Add commentMore actions
                    mMetricsFeatureProvider.action(mActivity,
                            SettingsEnums.ACTION_SETTINGS_DISABLE_APP);
                    AsyncTask.execute(new DisableChangerRunnable(mPm, mAppEntry.info.packageName,
                            PackageManager.COMPONENT_ENABLED_STATE_DISABLED_USER));
            ...
            case ButtonActionDialogFragment.DialogType.FORCE_STOP:
                requireAuthAndExecute(() -> {
                    forceStopPackage(mAppEntry.info.packageName);Add commentMore actions
                });
                break;
        }
    }
    ...
    void uninstallPkg(String packageName, boolean allUsers, boolean andDisable) {
        requireAuthAndExecute(() -> {
            ...
            mMetricsFeatureProvider.action(
                    mActivity, SettingsEnums.ACTION_SETTINGS_UNINSTALL_APP);Add commentMore actions
